...
- `data/assets.ts` — every image URL, hot-linked from the source CDN at `https://memovo.com/wp-content/uploads` (the host the photos still live on). No images are stored in this repo.
...
`next.config.ts` allowlists the two CDN hosts the photos load from, `memovo.com` and `cdn.weglot.com`, in `images.remotePatterns`, and the built-in optimizer caches each file on disk for 30 days. Requesting the originals directly from the browser makes the CDN drop responses once a page fires 30+ of them at once, so requests go through the optimizer.
